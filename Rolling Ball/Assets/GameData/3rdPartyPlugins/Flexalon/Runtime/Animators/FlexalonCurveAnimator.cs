...
namespace Flexalon
{
    ...
    public class FlexalonCurveAnimator : MonoBehaviour, TransformUpdater
    {
        ...
        public bool UpdateRectSize(FlexalonNode node, Vector2 size)
        {
            ...
            if (!_animateScale || _rectSizeTime > _curve.keys[_curve.keys.Length - 1].time)
            {
            ...
            {
                _rectTransform.sizeDelta = Vector2.Lerp(_startRectSize, _endRectSize, _curve.Evaluate(_rectSizeTime));
                return false;
            }
        }
    }
