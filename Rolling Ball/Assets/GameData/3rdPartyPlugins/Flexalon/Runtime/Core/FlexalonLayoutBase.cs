    ...
    public abstract class LayoutBase : FlexalonComponent, Layout
    {
        ...
        private void DetectChanges()
        {
            ...
            for (int i = 0; i < _node.Children.Count; i++)
            {
                ...
                else if (!childNode.GameObject.transform.IsChildOf(transform) || childNode.IsDragging || childNode.Constraint != null)
                {
                    i--;
                    childNode.Detach();
            ...
            foreach (Transform child in transform)
            {
                ...
                if (childNode.IsDragging || childNode.Constraint != null)
                {
                    continue;
                }
