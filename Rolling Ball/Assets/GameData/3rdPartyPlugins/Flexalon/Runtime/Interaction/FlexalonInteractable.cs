...
namespace Flexalon
{
    ...
    public class FlexalonInteractable : MonoBehaviour
    {
        ...
        private bool TryFindNearestDragTarget(FlexalonDragTarget currentDragTarget, out FlexalonDragTarget dragTarget, out Transform nearestChild)
        {
            ...
            GetInsertPositionAndRadius(_node, out var worldInsertPosition, out var worldInsertRadius);
        ...
        private void GetInsertPositionAndRadius(FlexalonNode node, out Vector3 position, out float radius)
        {
            var worldBoxScale = node.GetWorldBoxScale(true);
            var boxToWorld = Matrix4x4.TRS(node.GetWorldBoxPosition(worldBoxScale, false), transform.rotation, worldBoxScale);
            var scale = transform.lossyScale;
            radius = _insertRadius * Mathf.Max(scale.x, scale.y, scale.z);
            position = boxToWorld.MultiplyPoint(Vector3.zero);
        }
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (node != null && _draggable)
            {
                ...
                GetInsertPositionAndRadius(node, out var insertPosition, out var insertRadius);
