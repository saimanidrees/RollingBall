...
using UnityEngine;
public class EndingWithBlocks : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            foreach (var block in positionsToLerp)
            {
    ...
    IEnumerator LerpBall(int index, GameObject ball)
    {
        var delay = 0.5f;
        if (ballController)
        {
        ...
        while (!triggered)
        {
            triggered = positionsToLerp[index].GetComponent<DestructableCubePiece>().scattered;
            if (ballController) 
                ballController.speed = 800;          
            yield return null;            
        }
